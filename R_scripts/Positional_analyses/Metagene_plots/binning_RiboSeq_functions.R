...
plot_binned_lines <- function(df, SD = T, control = control, treatment = treatment, colors = colors) {
...
plot_binned_all_replicates <- function(alist, control = control, treatment = treatment, colors = colors) {
...
plot_positional_lines <- function(df, SD = T, control = control, treatment = treatment, colors = colors) {
...
plot_single_nt_lines <- function(df, SD = T, plot_ends = F, control = control, treatment = treatment, colors = colors) {
...
plot_binned_delta <- function(df, SD = T) {
  ...
  df[df$region == "UTR5",] %>%
    ...
    geom_col(fill = KO_color)+
    ylim(ylims)+
    UTR5_theme+
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> UTR5_delta_plot
  ...
  df[df$region == "CDS",] %>%
    ...
    ylim(ylims)+
    CDS_theme+
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> CDS_delta_plot
  ...
  df[df$region == "UTR3",] %>%
    ...
    ylim(ylims)+
    UTR3_theme+
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> UTR3_delta_plot
...
plot_single_nt_delta <- function(df, SD = T, plot_ends = F) {
  ...
  df[df$region == "UTR5" & df$window > 0,] %>%
    ...
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> UTR5_start_plot
  ...
  df[df$region == "UTR5" & df$window < 0,] %>%
    ...
    theme(axis.text.x = element_text(size = 14),
          axis.title.x = element_text(size = 16))+
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> UTR5_end_plot
  ...
  df[df$region == "CDS" & df$window > 0,] %>%
    ...
    theme(axis.text.x = element_text(size = 14),
          axis.title.x = element_text(size = 16))+
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> CDS_start_plot
  ...
  df[df$region == "CDS" & df$window < 0,] %>%
    ...
    theme(axis.text.x = element_text(size = 14),
          axis.title.x = element_text(size = 16))+
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> CDS_end_plot
  ...
  df[df$region == "UTR3"  & df$window > 0,] %>%
    ...
    theme(axis.text.x = element_text(size = 14),
          axis.title.x = element_text(size = 16))+
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> UTR3_start_plot
  ...
  df[df$region == "UTR3"  & df$window < 0,] %>%
    ...
    theme(axis.text.x = element_text(size = 14),
          axis.title.x = element_text(size = 16))+
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> UTR3_stop_plot
...
plot_positional_delta <- function(df, SD = T) {
  ...
  df %>%
    ...
    {if(SD)geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3, colour = NA)} -> delta_plot
...
plot_subset <- function(IDs, subset, sub_dir,
                        binned_value, single_nt_value, control = control, treatment = treatment, colors = colors,
  ...
  if (plot_binned == T) {
    ...
    #plot lines
    subset_binned_line_plots <- plot_binned_lines(summarised_subset_binned, SD = SD, control = control, treatment = treatment, colors = colors)
    
    png(filename = file.path(parent_dir, "plots/binned_plots", sub_dir, paste(subset, binned_value, "lines.png")), width = 1000, height = 230)
    grid.arrange(subset_binned_line_plots[[1]], subset_binned_line_plots[[2]], subset_binned_line_plots[[3]], nrow = 1, widths = c(1,2,1.5),
                 top = textGrob(paste(Tissue, TMT, subset),gp=gpar(fontsize=16,font=2)))
    dev.off()
    ...
    if (plot_delta == T) {
      ...
      png(filename = file.path(parent_dir, "plots/binned_plots", sub_dir, paste(subset, binned_value, "delta.png")), width = 1000, height = 230)
    ...
    if (plot_replicates == T) {
      subset_binned_line_plots_all_replicates <- plot_binned_all_replicates(summarised_subset_binned_list, control = control, treatment = treatment, colors = colors)
      
      png(filename = file.path(parent_dir, "plots/binned_plots", sub_dir, paste(subset, binned_value, "lines all replicates.png")), width = 1000, height = 230)
  ...
  if (plot_single_nt == T) {
    ...
    #plot lines
    subset_single_nt_line_plots <- plot_single_nt_lines(summarised_subset_single_nt, SD = SD, control = control, treatment = treatment, colors = colors)
    
    png(filename = file.path(parent_dir, "plots/binned_plots", sub_dir, paste(subset, single_nt_value, "lines.png")), width = 1300, height = 300)
    ...
    if (plot_delta == T) {
      ...
      png(filename = file.path(parent_dir, "plots/binned_plots", sub_dir, paste(subset, single_nt_value, "delta.png")), width = 1300, height = 230)
  ...
  if (plot_positional == T) {
    ...
    subset_positional_line_plots <- plot_positional_lines(df = summarised_positional_subset_binned, SD = SD, control = control, treatment = treatment, colors = colors)
    
    png(filename = file.path(parent_dir, "plots/binned_plots", sub_dir, paste(subset, "binned positional lines.png")), width = 500, height = 230)
    ...
    if (plot_delta == T) {
      ...
      png(filename = file.path(parent_dir, "plots/binned_plots", sub_dir, paste(subset, "binned positional delta.png")), width = 500, height = 230)
...
plot_GSEA_binned <- function(GSEA_set, pathway, subdir,
                             ...
                             plot_binned = T, plot_single_nt = F, plot_positional = F,
                             sub_dir, control = control, treatment = treatment, colors = colors, paired_data = T, SD = T, plot_delta = T) {
  ...
  plot_subset(IDs = GSEA_transcript_IDs, subset = pathway, sub_dir= sub_dir,
              binned_value = binned_value, single_nt_value = single_nt_value, control = control, treatment = treatment, colors = colors,
...
plot_single_transcripts <- function(gene, dir, colors = colors,
  ...
  if (plot_binned == T) {
    ...
    if (plot_replicates == F) {
      ...
      binned_line_plots <- plot_binned_lines(df = summarised_binned, SD = SD, control = control, treatment = treatment, colors = colors)
      
      png(filename = file.path(parent_dir, "plots/binned_plots/single_transcripts", dir, paste(Tissue, TMT, gene, "binned normalised lines.png")), width = 1000, height = 230)
      grid.arrange(binned_line_plots[[1]], binned_line_plots[[2]], binned_line_plots[[3]], nrow = 1, widths = c(1,2,1.5),
                   top = textGrob(paste(Tissue, TMT, gene, "binned"),gp=gpar(fontsize=16,font=2)))
      dev.off()
      ...
      binned_line_plots_all_replicates <- plot_binned_all_replicates(summarised_binned_list, control = control, treatment = treatment, colors = colors)
      
      png(filename = file.path(parent_dir, "plots/binned_plots/single_transcripts", dir, paste(Tissue, TMT, gene, "binned normalised lines all replicates.png")), width = 1000, height = 230)
    ...
    if (plot_delta == T) {
      ...
      png(filename = file.path(parent_dir, "plots/binned_plots/single_transcripts", dir, paste(Tissue, TMT, gene, "binned normalised delta.png")), width = 1000, height = 230)
      grid.arrange(binned_delta_plots[[1]], binned_delta_plots[[2]], binned_delta_plots[[3]],
                   nrow = 1, widths = c(1,2,1), top = textGrob(paste(Tissue, TMT, gene, "binned"),gp=gpar(fontsize=16,font=2)))
      dev.off()
    }
  }
  ...
  if (plot_single_nt == T) {
    ...
    #plot
    single_nt_line_plots <- plot_single_nt_lines(summarised_single_nt, SD=SD, control = control, treatment = treatment, colors = colors)
    
    png(filename = file.path(parent_dir, "plots/binned_plots/single_transcripts", dir, paste(Tissue, TMT, gene, "single nt normalised lines.png")), width = 1000, height = 230)
    grid.arrange(single_nt_line_plots[[1]], single_nt_line_plots[[2]], single_nt_line_plots[[3]], single_nt_line_plots[[4]], nrow = 1, widths = c(rep(1,3),1.2),
                 top = textGrob(paste(Tissue, TMT, gene, "single nt."),gp=gpar(fontsize=16,font=2)))
    dev.off()
    ...
    if (plot_delta == T) {
      ...
      png(filename = file.path(parent_dir, "plots/binned_plots/single_transcripts", dir, paste(Tissue, TMT, gene, "single nt normalised delta.png")), width = 1000, height = 230)
      grid.arrange(single_nt_delta_plots[[1]], single_nt_delta_plots[[2]], single_nt_delta_plots[[3]], single_nt_delta_plots[[4]], nrow = 1, widths = c(rep(1,3),1.2),
                   top = textGrob(paste(Tissue, TMT, gene, "single nt."),gp=gpar(fontsize=16,font=2)))
      dev.off()
    }
  }
  ...
  if (plot_codons == T) {
    ...
    #plot
    codon_line_plots <- plot_positional_lines(summarised_codon, SD=SD, control = control, treatment = treatment, colors = colors)
    
    png(filename = file.path(parent_dir, "plots/binned_plots/single_transcripts", dir, paste(Tissue, TMT, gene, "single codon normalised lines.png")), width = 1000, height = 230)
    ...
    if (plot_delta == T) {
      ...
      png(filename = file.path(parent_dir, "plots/binned_plots/single_transcripts", dir, paste(Tissue, TMT, gene, "single codon normalised delta.png")), width = 1000, height = 230)
