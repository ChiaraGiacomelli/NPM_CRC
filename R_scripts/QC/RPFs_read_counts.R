...
RPF_counts$condition <- as.factor(RPF_counts$condition)
RPF_counts$condition <- fct_rev(RPF_counts$condition)
...
RPF_counts %>%
  ...
  ggtitle("RPFs")+
  myTheme+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0.5)) -> RPF_aligments_plot

png(filename = file.path(parent_dir, "plots/read_counts_summary/RPF_aligments_plot.png"), width = 900, height = 550)
